...
CMD:graffiti(playerid, params[]) {
	...
	if(!CREW_RANK_INFO[ PlayerTemp[playerid][pt_CREW_INDEX] ][ PI[playerid][pi_CREW_RANK] ][crew_rank_PERMISSION][CREW_RANK_GRAFFITI]) return SendClientMessagef(playerid, -1, "No tienes permiso para crear o borrar graffitis.");
	...
	if(TERRITORIES[ PlayerTemp[playerid][pt_LAST_TERRITORY] ][territory_CREW_ID] != PI[playerid][pi_CREW]) return SendClientMessagef(playerid, -1, "No estás en un territorio de tu banda.");			
    if(PI[playerid][pi_LEVEL] < 6) return SendClientMessagef(playerid, -1, "Debes ser nivel 6 o superior para usar los graffitis.");
    ...
    PlayerTemp[playerid][pt_DIALOG_ID] = DIALOG_GRAFFITI_OPTIONS;

    new string[128];
    format(string, sizeof string, "1. Crear graffiti (%d/%d)\n2. Editar graffiti", CountGraffitisInZone(PlayerTemp[playerid][pt_LAST_TERRITORY]), MAX_GRAFFITI_PER_ZONE);
    ...
    switch(dialogid) {
        case DIALOG_GRAFFITI_OPTIONS: {
            if(response) {
                switch(listitem) {
                    case 0: {
                        new territoryIndex = PlayerTemp[playerid][pt_PLAYER_TERRITORY_PRO];
                        if(CountGraffitisInZone(territoryIndex) >= MAX_GRAFFITI_PER_ZONE) SendClientMessagef(playerid, -1, "No se pueden crear más graffitis en este territorio.");
                        else {
                            ...
                            inline OnQueryLoaded()
	                        {
                                ...
                                if(id > 0) {
                                    new objectid = CreateGraffitiFromInfo(id, pos[0], pos[1], pos[2], 0.0, 0.0, angle, interior, world, 19479, "Graffiti", "Arial", 30, 0, 0xFFFFFFFF, TERRITORIES[territoryIndex][territory_ID], PI[playerid][pi_CREW], PI[playerid][pi_ID], territoryIndex, PlayerTemp[playerid][pt_CREW_INDEX]);
                                    Streamer_Update(playerid, STREAMER_TYPE_OBJECT);
                                    PlayerTemp[playerid][pt_SELECTED_GRAFFITI] = objectid;
        ...
        case DIALOG_GRAFFITI_EDIT: {
            if(response) {
                switch(listitem) {
                    case 0: EditDynamicObject(playerid, PlayerTemp[playerid][pt_SELECTED_GRAFFITI]);
                    case 1: {
                        ShowEditGraffitiTextDialog(playerid);
                    }
                    case 2: {
                        new info[4];
	                    Streamer_GetArrayData(STREAMER_TYPE_OBJECT, PlayerTemp[playerid][pt_SELECTED_GRAFFITI], E_STREAMER_EXTRA_ID, info);
                        ...
                        if(index != -1) {
                            ...
                            SendClientMessagef(playerid, -1, "Se ha eliminado el graffiti.");
                        }
                    }
                }
            }
        ...
        case DIALOG_GRAFFITI_TEXT_OPTIONS: {
            if(response) {
                switch(listitem) {
                    case 0: { //texto
                        PlayerTemp[playerid][pt_DIALOG_RESPONDED] = false;
                        PlayerTemp[playerid][pt_DIALOG_ID] = DIALOG_GRAFFITI_ETEXT_TEXT;
                    ...
                    case 1: { //fuente
                        PlayerTemp[playerid][pt_DIALOG_RESPONDED] = false;
                        PlayerTemp[playerid][pt_DIALOG_ID] = DIALOG_GRAFFITI_ETEXT_FONT;
                    ...
                    case 2: { //tamaño
                        PlayerTemp[playerid][pt_DIALOG_RESPONDED] = false;
                        PlayerTemp[playerid][pt_DIALOG_ID] = DIALOG_GRAFFITI_ETEXT_SIZE;
                    ...
                    case 4: { //color
                        ...
                        PlayerTemp[playerid][pt_DIALOG_RESPONDED] = false;
                        PlayerTemp[playerid][pt_DIALOG_ID] = DIALOG_GRAFFITI_ETEXT_COLOR;
        ...
        case DIALOG_GRAFFITI_ETEXT_TEXT: {
            if(response) {
                new value[128];
                if(sscanf(inputtext, "s[128]", value)) return SendClientMessagef(playerid, -1, "Valor no válido.");
        ...
        case DIALOG_GRAFFITI_ETEXT_FONT: {
            if(response) {
                new value[24];
                if(sscanf(inputtext, "s[24]", value)) return SendClientMessagef(playerid, -1, "Valor no válido.");
        ...
        case DIALOG_GRAFFITI_ETEXT_SIZE: {
            if(response) {
                new value;
                if(sscanf(inputtext, "d", value)) return SendClientMessagef(playerid, -1, "Valor no válido.");
                if(value <= 0 || value > 200) return SendClientMessagef(playerid, -1, "Valor no válido.");
...
hook OnPlayerSelectDynObject(playerid, STREAMER_TAG_OBJECT objectid, modelid, Float:x, Float:y, Float:z) {
    ...
    if(info[0] == OBJECT_TYPE_GRAFFITI) {
        ...
        if(PlayerTemp[playerid][pt_PLAYER_TERRITORY_PRO] == territoryIndex && PlayerTemp[playerid][pt_CREW_INDEX] == crewIndex && CREW_RANK_INFO[crewIndex][ PI[playerid][pi_CREW_RANK] ][crew_rank_PERMISSION][CREW_RANK_GRAFFITI]) {
            CancelEdit(playerid);
            PlayerTemp[playerid][pt_SELECTED_GRAFFITI] = objectid;
...
hook OnPlayerEditDynObject(playerid, STREAMER_TAG_OBJECT objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz) {
    ...
    if(info[0] == OBJECT_TYPE_GRAFFITI && objectid == PlayerTemp[playerid][pt_SELECTED_GRAFFITI]) {
        switch(response) {
            case EDIT_RESPONSE_CANCEL: {
                ...
                SendClientMessagef(playerid, -1, "Has cancelado la edición y el objeto a vuelto a su posición anterior.");
                ShowDialog(playerid, DIALOG_GRAFFITI_EDIT);
            }
            case EDIT_RESPONSE_FINAL: {
                ...
                SendClientMessagef(playerid, -1, "La posición del graffiti se ha actualizado correctamente.");
                ShowDialog(playerid, DIALOG_GRAFFITI_EDIT);
            }
        }
        return Y_HOOKS_BREAK_RETURN_1;
...
ShowEditGraffitiTextDialog(playerid) {
    ...
    if(index != -1) {
        ...
        PlayerTemp[playerid][pt_DIALOG_RESPONDED] = false;
        PlayerTemp[playerid][pt_DIALOG_ID] = DIALOG_GRAFFITI_TEXT_OPTIONS;
