...
CMD:barco(playerid, params[]) {
    ...
    if(Vehicle_IsBoat(vehicleid)) {
        new info[3];
        Streamer_GetArrayData(STREAMER_TYPE_AREA, PlayerTemp[playerid][pt_LAST_AREA_ID], E_STREAMER_EXTRA_ID, info);
        if(info[0] != AREA_TYPE_WATERVEH_STATION || !IsPlayerInDynamicArea(playerid, PlayerTemp[playerid][pt_LAST_AREA_ID])) return SendClientMessagef(playerid, -1, "No estás en el lugar adecuado.");
        ...
        PlayerTemp[playerid][pt_DIALOG_ID] = DIALOG_BOAT_OPTIONS;
            ...
            if(response) {
                switch(listitem) {
                    ...
                    case 1, 2: {
                        ...
                        if(PLAYER_VEHICLES[vehicleid][player_vehicle_VALID] && PLAYER_VEHICLES[vehicleid][player_vehicle_OWNER_ID] == PI[playerid][pi_ID]) {
                            PlayerTemp[playerid][pt_MECHANIC_COLOR_SLOT] = listitem - 1;
