...
hook OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ) {
    if(PI[playerid][pi_CREW]) {
        new now = gettime(), crewIndex = PlayerTemp[playerid][pt_CREW_INDEX];
        new enterExitIndex = PlayerTemp[playerid][pt_INTERIOR_INDEX];
        if(enterExitIndex != -1 && ENTER_EXIT[enterExitIndex][ee_ROBBABLE]) {
            if(ENTER_EXIT[enterExitIndex][ee_TIMER] == -1) {
                ...
                if(crewIndex == ENTER_EXIT[enterExitIndex][ee_LAST_CREW_INDEX_ROBBERY]) {
                    if(now > PlayerTemp[playerid][pt_LAST_SHOT_ROBBERY] + 1) {
                        PlayerTemp[playerid][pt_LAST_SHOT_ROBBERY] = now;
...
hook OnPlayerEnterInterior(playerid, index) {
    if(ENTER_EXIT[index][ee_TIMER] != -1) {
        ...
        if(PI[playerid][pi_CREW] && ENTER_EXIT[index][ee_LAST_CREW_INDEX_ROBBERY] == PlayerTemp[playerid][pt_CREW_INDEX]) {
	...
	for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
	{
		if(IsPlayerConnected(i))
		{
			if(PI[i][pi_CREW])
			{
				...
				{
					if(PI[i][pi_STATE] == ROLEPLAY_STATE_INTERIOR && PlayerTemp[i][pt_GAME_STATE] == GAME_STATE_NORMAL)
					{
						if(PlayerTemp[i][pt_INTERIOR_INDEX] == enter_exit_index)
						{
							count ++;
						}
					}
...
StartShopAttack(crew_index, enter_exit_index)
{
    ...
    for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
	{
		if(IsPlayerConnected(i))
		{
			if(PI[i][pi_CREW] && PlayerTemp[i][pt_CREW_INDEX] == crew_index && PlayerTemp[i][pt_INTERIOR_INDEX] == enter_exit_index)
			{
        ...
        for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
        {
            if(IsPlayerConnected(i))
            {
                if(PlayerTemp[i][pt_INTERIOR_INDEX] == enter_exit_index)
                {
                    ...
                    if(PI[i][pi_CREW] && PlayerTemp[i][pt_CREW_INDEX] == crew_index) {
